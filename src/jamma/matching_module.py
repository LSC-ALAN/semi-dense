...
class PatchFlowEstimator(nn.Module):
    ...
    def forward(self, feat_f0_unfold, feat_f1_unfold, data):
        ...
        W_f = self.W_f
        B, C, H, W = f1.shape
        x = torch.cat([f1, f2], dim=1)            # (B, 2C, 5, 5)
        x = self.input_proj(x)                    # (B, hidden_dim, 5, 5)
        x = x.flatten(2).permute(0, 2, 1)         # (B, 25, hidden_dim)
        x = x + self.pos_embed                    # 加上learnable位置编码

        x = self.transformer(x)                   # (B, 25, hidden_dim)
        out = self.to_flow_certainty(x)           # (B, 25, 3)
        out = out.permute(0, 2, 1).reshape(B, 3, H, W)  # (B, 3, 5, 5)

        flow = out[:, :2]        # (B, 2, 5, 5)
        certainty = out[:, 2:]   # (B, 1, 5, 5)
