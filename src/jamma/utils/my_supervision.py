...
def spvs_coarse(data, config):
    ...
    # plt.imshow(data['depth0'][0].cpu().numpy())  # 检查第一张深度图
    # plt.colorbar()
    # plt.show()
    # 1. misc
...
def spvs_fine(data, config):
    ...
    label0 = torch.ones_like(w_pt0_f[..., :1])  # shape: (s, 25, 1)
    label1 = torch.ones_like(w_pt0_f[..., :1])  # shape: (s, 25, 1)

    M = w_pt0_f.shape[0]

    # corner case: out of boundary
    def out_bound_mask(pt, w, h):
        return (pt[..., 0] < -w/2) + (pt[..., 0] >= w/2) + (pt[..., 1] < -h/2) + (pt[..., 1] >= h/2)
    label0[out_bound_mask(w_pt0_f, W_f, W_f)] = 0
    label1[out_bound_mask(w_pt1_f, W_f, W_f)] = 0
    # label:(s,25,2) 2level
    data.update({"label0": label0,
                "label1": label1,
                "w_pt0_f": w_pt0_f,
                "w_pt1_f": w_pt1_f,
                })